...
function handleCxFilter_getParams() {
	...
	$('[name^="cx_filter"]').each(function() {
		if ($(this).attr("use-param") == 1) {
			var cmpName = $(this).attr("name");
			var cmpValue = $(this).val();
	
			// Clean name
			cmpName = cmpName.replace("cx_filter[", "cxfilter_").replace("]", "");
	
			// Set param
			params += `&${cmpName}=${cmpValue}`;
		};
	});
...
function handleCxFilter_setXlsx(dataTableName) {
	var dataCount = $("#" + dataTableName).DataTable().data().count();
	if (dataCount > 0) {
