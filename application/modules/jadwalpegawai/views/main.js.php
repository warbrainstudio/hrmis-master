<script type="text/javascript">
  $(document).ready(function() {
    ...
    if ($("#" + _table)[0]) {
      var table_jadwal = $("#" + _table).DataTable({
        processing: true,
        serverSide: true,
        ajax: {
          url: "<?php echo base_url('jadwalpegawai/ajax_get_all/') ?>",
          type: "get"
        },
        columns: [{
            data: null,
    ...
    $("#" + _table).on("click", "a.action-edit", function(e) {
      e.preventDefault();
      resetForm();
      var temp = table_jadwal.row($(this).closest('tr')).data();
    ...
    $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
      ...
      $.ajax({
        type: "post",
        url: "<?php echo base_url('jadwalpegawai/ajax_save/') ?>" + _key,
    ...
    $("#" + _table).on("click", "a.action-delete", function(e) {
      ...
      swal({
        ...
        if (result.value) {
          $.ajax({
            type: "delete",
            url: "<?php echo base_url('jadwalpegawai/ajax_delete/') ?>" + temp.id,
            dataType: "json",
            success: function(response) {
              if (response.status) {
                $("#" + _table).DataTable().ajax.reload(null, false);
