<script type="text/javascript">
    $(document).ready(function() {
        var _key = "<?= $key ?>";
        var _section = "skspk";
        var _table_master = "table-skspk";
        var _modal_view = "modal-view-skspk";
        var _modal_form = "modal-form-skspk";
        var _form = "form-skspk";
        ...
        // Init on load
        initSelect2_enter(".skspk-pegawai_id", "Cari dengan NRP / Nama Lengkap...", "<?= base_url('ref/ajax_search_pegawai') ?>", formatSelect2Result_pegawai);
                            ...
                            render: function(data, type, row, meta) {
                                return `
                                    <div class="action" style="display: flex; flex-direction: row;">
                                        <a href="<?= base_url('skspk/detail?ref=') ?>${row.id}" modal-id="modal-view-skspk" class="btn btn-sm btn-success x-load-modal-partial" title="Rincian"><i class="zmdi zmdi-eye"></i></a>&nbsp;
                                        <a href="<?= base_url('skspk/input?ref=') ?>${row.id}" modal-id="modal-form-skspk" class="btn btn-sm btn-light x-load-modal-partial" title="Ubah"><i class="zmdi zmdi-edit"></i> Ubah</a>&nbsp;
                                        <a href="<?= base_url('skspk/delete?ref=') ?>${row.id}" class="btn btn-sm btn-danger action-delete" title="Hapus"><i class="zmdi zmdi-delete"></i> Hapus</a>
                                    </div>
                                `;
                            }
                        }
        ...
        // Handle submit
        $(document).on("click", `#${_modal_form} .skspk-action-save`, function(e) {
            e.preventDefault();
            tinyMCE.triggerSave();
            if (table_master) {
                swal({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            type: "post",
                            url: "<?php echo base_url('skspk/ajax_save/') ?>",
                            data: formData,
                            dataType: "json",
            ...
            if (table_master) {
                ...
                swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "delete",
                            url: "<?php echo base_url('skspk/ajax_delete/') ?>" + temp.id,
                            dataType: "json",
                            success: function(response) {
                                if (response.status) {
                                    table_master.ajax.reload(null, false);
        ...
        function load_select2DefaultValue() {
            setTimeout(() => {
                if (_is_first_load === true && (_pegawai_id != "" && _pegawai_namaLengkap != "")) {
                    ...
                    $(".skspk-pegawai_id").append(optionPegawai).trigger("change");
                };
            }, 300);
        };
    });
