<script type="text/javascript">
  $(document).ready(function() {
    ...
    var _key = "";
    var _section = "kategoripegawai";
    var _table = "table-kategoripegawai";
    var _modal = "modal-form-kategoripegawai";
    var _form = "form-kategoripegawai";
    ...
    if ($("#" + _table)[0]) {
      var table_master = $("#" + _table).DataTable({
        ...
        serverSide: true,
        ajax: {
          url: "<?php echo base_url('kategoripegawai/ajax_get_all/') ?>",
          type: "get"
        },
        columns: [{
            data: null,
    ...
    $("#" + _table).on("click", "a.action-edit", function(e) {
      ...
      $.each(temp, function(key, item) {
        var _xtype = $(`input[name='${key}']`).attr("type");
        ...
        // Radio component
        if (_xtype === 'radio') {
          $(`#${_form} input[name='${key}'][value='${item}']`).prop("checked", true);
        };
      });
    });
    ...
    $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
      ...
      $.ajax({
        type: "post",
        url: "<?php echo base_url('kategoripegawai/ajax_save/') ?>" + _key,
    ...
    $("#" + _table).on("click", "a.action-delete", function(e) {
      ...
      swal({
        ...
        if (result.value) {
          $.ajax({
            type: "delete",
            url: "<?php echo base_url('kategoripegawai/ajax_delete/') ?>" + temp.id,
            dataType: "json",
            success: function(response) {
              if (response.status) {
                $("#" + _table).DataTable().ajax.reload(null, false);
