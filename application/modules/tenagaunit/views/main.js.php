<script type="text/javascript">
  $(document).ready(function() {
    ...
    var _key = "";
    var _section = "tenagaunit";
    var _table = "table-tenagaunit";
    var _modal = "modal-form-tenagaunit";
    var _form = "form-tenagaunit";
    ...
    if ($("#" + _table)[0]) {
      var table_master = $("#" + _table).DataTable({
        ...
        serverSide: true,
        ajax: {
          url: "<?php echo base_url('tenagaunit/ajax_get_all/') ?>",
          type: "get"
        },
        columns: [{
            data: null,
          ...
          {
            data: "kode_tenaga_unit"
          },
          {
            data: "nama_tenaga_unit"
          },
          {
            data: null,
            className: "center",
    ...
    $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
      ...
      $.ajax({
        type: "post",
        url: "<?php echo base_url('tenagaunit/ajax_save/') ?>" + _key,
    ...
    $("#" + _table).on("click", "a.action-delete", function(e) {
      ...
      swal({
        ...
        if (result.value) {
          $.ajax({
            type: "delete",
            url: "<?php echo base_url('tenagaunit/ajax_delete/') ?>" + temp.id,
            dataType: "json",
            success: function(response) {
              if (response.status) {
                $("#" + _table).DataTable().ajax.reload(null, false);
