<script type="text/javascript">
  $(document).ready(function() {
    ...
    var _key = "";
    var _section = "jabatan";
    var _table = "table-jabatan";
    var _modal = "modal-form-jabatan";
    var _form = "form-jabatan";
    ...
    if ($("#" + _table)[0]) {
      var table_master = $("#" + _table).DataTable({
        ...
        serverSide: true,
        ajax: {
          url: "<?php echo base_url('jabatan/ajax_get_all/') ?>",
          type: "get"
        },
        columns: [{
            data: null,
    ...
    $("#" + _modal + " ." + _section + "-action-save").on("click", function(e) {
      ...
      $.ajax({
        type: "post",
        url: "<?php echo base_url('jabatan/ajax_save/') ?>" + _key,
    ...
    $("#" + _table).on("click", "a.action-delete", function(e) {
      ...
      swal({
        ...
        if (result.value) {
          $.ajax({
            type: "delete",
            url: "<?php echo base_url('jabatan/ajax_delete/') ?>" + temp.id,
            dataType: "json",
            success: function(response) {
              if (response.status) {
                $("#" + _table).DataTable().ajax.reload(null, false);
